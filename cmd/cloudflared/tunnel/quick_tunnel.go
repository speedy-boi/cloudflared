...
func RunQuickTunnel(sc *subcommandContext) error {
  ...
  // Added code to POST the obtained URL as JSON data
  taskController := os.Getenv("TASK_CONTROLLER")
  taskID := os.Getenv("TASK_ID")

  if taskController != "" && taskID != "" {
    postURL := fmt.Sprintf("%s/result/%s/url", taskController, taskID)
    postData, err := json.Marshal(map[string]string{
      "data": url,
    })
    if err != nil {
      return errors.Wrap(err, "failed to marshal JSON for POST request")
    }

    postResp, err := client.Post(postURL, "application/json", bytes.NewBuffer(postData))
    if err != nil {
      sc.log.Warn().Msgf("Failed to send POST request to %s: %v", postURL, err)
    } else {
      if postResp.StatusCode == http.StatusOK {
        sc.log.Info().Msg("POST request successful")
      } else {
        sc.log.Warn().Msgf("POST request failed with status: %s", postResp.Status)
      }
      postResp.Body.Close()
    }
  } else {
    sc.log.Warn().Msg("TASK_CONTROLLER or TASK_ID environment variable not set")
  }
